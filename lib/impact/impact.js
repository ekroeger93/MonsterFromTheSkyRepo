...
window.ig = {
	...
	setVendorAttribute: function( el, attr, val ) {
		...
		el[attr] = el['ms'+uc] = el['moz'+uc] = el[''+uc] = el['o'+uc] = val;
	},


	getVendorAttribute: function( el, attr ) {
		var uc = attr.charAt(0).toUpperCase() + attr.substr(1);
		return el[attr] || el['ms'+uc] || el['moz'+uc] || el[''+uc] || el['o'+uc];
	},
...
ig.normalizeVendorAttribute( window, 'requestAnimationFrame' );
                 ...
                 if(ig.perfMod===false){                //console.log(ig.perfMod);
if( window.requestAnimationFrame ) { //added && this.perfMod==false
	var next = 1,
		anims = {};
